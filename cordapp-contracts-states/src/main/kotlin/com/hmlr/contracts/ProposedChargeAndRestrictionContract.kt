...
class ProposedChargeAndRestrictionContract: Contract {
    ...
    private fun verifyTransferLandTitle(tx: LedgerTransaction, setOfSigners: Set<PublicKey>) = requireThat {
        ...
        "Transaction must be signed by both the conveyancers, title issuer, settling party and revenueAndCustom Party" using(setOfSigners.size == 5 && setOfSigners.containsAll(listOf(inputLandAgreementState.sellerConveyancer.owningKey, inputLandAgreementState.buyerConveyancer.owningKey, inputLandTitleState.titleIssuer.owningKey, inputPaymentConfirmationState.settlingParty.owningKey, inputLandTitleState.revenueAndCustom.owningKey)))
        outputProposedChargesAndRestrictionsState.restrictions.forEach {
            require(it.action == ActionOnRestriction.NO_ACTION) { "Restrictions should have no actions when issued on ledger" }
            require(!it.consentGiven) { "Consent given should be false on Restrictions when issued on ledger" }
            when (it) {
