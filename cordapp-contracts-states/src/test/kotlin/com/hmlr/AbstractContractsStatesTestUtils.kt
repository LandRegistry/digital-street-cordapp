...
abstract class AbstractContractsStatesTestUtils {
    ...
    var HMRC = TestIdentity(CordaX500Name(organisation = "HMRC", locality = "TestVillage", country = "US"))
    ...
    val other = CustomParty("John", "Smith", "125464", location, UserType.INDIVIDUAL, "seller@example.com", "+447700900354", true, signature = null, publicKey = wrongSellerPublicKey)
    ...
    val buyer = CustomParty("David", "Jones", "543545", location, UserType.INDIVIDUAL, "buyer@example.com", "+447700900354", true, signature = null, publicKey = buyerPublicKey)
    ...
    val requestIssuanceState = RequestIssuanceState(titleId, ALICE.party, HMRC.party, BOB.party, seller, RequestIssuanceStatus.PENDING)
    var agreementState = LandAgreementState(titleId, buyer, seller, CHARLIE.party, BOB.party, LocalDate.now(), LocalDate.now().atStartOfDay().plusDays(14).toInstant(ZoneOffset.UTC), 9.0, 1000.POUNDS, 50.POUNDS, null, 950.POUNDS, landTitleState.linearId.toString(), listOf(), TitleGuarantee.FULL, AgreementStatus.CREATED, false, null, "")
